...
// Kinetis Configuration, initializes processor to a default state.
//
// This version of sysinit.c contains a modified version of the original pll_init() function,
// originally found in the mgc.c source file. I moved that PLL startup code here and recoded it to
// assume use of a Teensy 3.x board. 8 Apr 14 KEL
...
//
// Declare these variables as extern in other modules if you need access to the actual system
// clock frequencies.
int32_t mcg_clk_hz;
int32_t mcg_clk_khz;
...
// This is the entry point following reset and low-level initialzation. This routine is responsible
// for system initialization and for invoking main().

// In the original Freescale Code Warrior example code, this routine lived in start.c.  I've moved
// it here so that a custom Teensy 3.1 project can consist of just three files; crt0.s, the main
// project file, and this file. 8 Apr 14  KEL
void start(void) {
  // Enable all of the port clocks. These have to be enabled to configure pin muxing options, so
...
void sysinit (void) {
  ...
  // NOTE: The PLL init will not configure the system clock dividers, so they must be configured
  // appropriately before calling the PLL init function to ensure that clocks remain in valid
  // ranges.
  SIM_CLKDIV1 = (0 |
  ...
  // Initialize PLL. PLL will be the source for MCG CLKOUT so the core, system, and flash clocks are
  // derived from it.
  ...
  // Use the value obtained from the pll_init function to define variables for the core clock in kHz
  // and also the peripheral clock. These variables can be used by other functions that need
  // awareness of the system frequency.
  ...
  // For debugging purposes, enable the trace clock and/or FB_CLK so that we'll be able to monitor
  // clocks and know the PLL is at the frequency that we expect.
  //fb_clk_init();
  //trace_clk_init();
}
...
// NOTE:  This code was heavily modified from the original supplied in the Freescale K20 example set
// (kinetis_50MHz_sc) as mgc.c.  This code is intended to run solely on the Teensy 3.x board, so the
// PLL initialization code makes assumptions about the hardware.  Specifically, the Teensy 3.x
// boards use a 16 MHz crystal, so there is no need for tests of the external oscillator value,
// originally passed in argument crystal_val. 8 Apr 14  KEL
//
...
//
// Parameters: prdiv_val: value to divide the external clock source by to create the desired PLL
...
int32_t pll_init(int8_t prdiv_val, int8_t vdiv_val) {
  ...
  // Configure the MCG_C2 register. The RANGE value is determined by the external frequency. Since
  // the RANGE parameter affects the FRDIV divide value it still needs to be set correctly even if
  // the oscillator is not being used.
  temp_reg = MCG_C2;
  // Clear fields before writing new values.
  temp_reg &= ~(MCG_C2_RANGE0_MASK | MCG_C2_HGO0_MASK | MCG_C2_EREFS0_MASK);
  temp_reg |= (MCG_C2_RANGE0(2) | (hgo_val << MCG_C2_HGO0_SHIFT) |
              (erefs_val << MCG_C2_EREFS0_SHIFT));
  MCG_C2 = temp_reg;

  // Removed tests around frdiv_val.  The frdiv_val is fixed at 4 because the Teensy always uses a
  // 16 MHz crystal.
  frdiv_val = 4;
  ...
  if (((MCG_S & MCG_S_CLKST_MASK) >> MCG_S_CLKST_SHIFT) != 0x2) return 0x1A;
  ...
  // Configure PLL.
  // Configure MCG_C5.
  // If the PLL is to run in STOP mode then the PLLSTEN bit needs to be OR'ed in here or in user
  // code.
  temp_reg = MCG_C5;
  ...
  // Check bit is really set and return with error if not set.
  if (!(MCG_S & MCG_S_PLLST_MASK)) return 0x16;
  ...
  // Check bit is really set and return with error if not set.
  if (!(MCG_S & MCG_S_LOCK0_MASK)) return 0x44;
...
//
// This routine was modified from the original in a Freescale Code Warrior example set. The
// original code was contained in a source file named wdog.c. That code disabled interrupts prior
// to unlocking the watchdog. Unfortunately, that code also blindly reenabled interrupts without
// regard to their state prior to entry.
//
